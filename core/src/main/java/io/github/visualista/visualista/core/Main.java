    ...
    public static void main(String[] args) {
        ...
        for (Scene scene : novel.getScenes()) {
            System.out.println("Dimension of scene in novel "
                    + scene.getGrid().getSize().toString());
        }
        ...
        String name = sc.next();
        actor.setName(name);
        System.out.println("The name of the created actor is "
                + actor.getName());

        sc.close();

        XStreamManager manager = new XStreamManager();
        CollectionSaver<Novel> saver = new CollectionSaver<Novel>(
                manager.getMainXStream(), new File("C:\\VISUALISTA\\"));
